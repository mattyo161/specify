 ...
 */
	...
	var setAsOpen = function (pane, skipCallback) {
		...
		if (state.initialized) {
			...
			sizeContent(pane, true); // true = force, even though 'isLayoutBusy'
		}
	...
	var bindStartSlidingEvent = function (pane, enable) {
		...
		if (trigger != "click" && trigger != "dblclick" && trigger != "mouseenter")
	...
	var bindStopSlidingEvents = function (pane, enable) {
		...
		,	$R		= $Rs[pane]
		;
		if (trigger == "mouseout") trigger = "mouseleave"
		...
		// make sure we have a valid event
		if (trigger != "mouseleave" && trigger != "click")
			trigger = o.slideTrigger_close = "mouseleave";
		...
		if (trigger == "mouseleave") {
			...
			$P[action]("mouseleave."+ sID, slideClosed);
			// cancel timer when mouse moves between 'pane' and 'resizer'
			$R[action]("mouseenter", cancelMouseOut); // don't need sID
	 ...
	 */
	var sizeContent = function (panes, force) {
		...
		$.each(panes.split(","), function (idx, pane) {
			...
			if (force || o.resizeContentWhileDragging || !_c.isLayoutBusy || m.above == undefined) {
				var
